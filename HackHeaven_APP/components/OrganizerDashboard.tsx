...
const MyEventsScreen = () => {
  ...
  const handleView = async (id: any) => {
    try {
      const url = `http://192.168.1.8:4003/api/organizer/getEventDetails?hackathon_id=${id}`;
  ...
  const handleGetEvents = async () => {
    ...
    try {
      ...
      if (loggedUserEmail) {
        const url = `http://192.168.1.8:4003/api/organizer/retrieveHackathonEventsByEmail?email=${encodeURIComponent(
          loggedUserEmail
        )}`;
  ...
  const handleDeleteEvent = async (id: any) => {
    try {
      const url = `http://192.168.1.8:4003/api/organizer/deleteHackathonEvent?hackathon_id=${id}`;
  ...
  const submitUpdate = async (id: any) => {
    ...
    const url = `http://192.168.1.8:4003/api/organizer/updateHackathonEvent/?hackathon_id=${id}`;
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.sectionTitle}>My Hackathon Events</Text>
